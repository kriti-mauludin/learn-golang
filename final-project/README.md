...
## Description
`final-project` is a boilerplate for Golang projects. The project structure follows the Clean Code Architecture ([Read here](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html)). This Skeleton made with **Fiber Framework**.  
...
1. Clone this repo
```sh
git clone https://github.com/kriti-mauludin/final-project.git
```
...
```sh
docker build -t final-project-api:1.0.1 -f ./deploy/docker/api/Dockerfile .
```
- Docker Build for Worker
```sh
docker build -t final-project-worker:1.0.1 -f ./deploy/docker/worker/Dockerfile .
```
